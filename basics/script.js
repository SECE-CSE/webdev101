...
function addTask() {
    ...
    taskItem.appendChild(deleteButton)
        ...
        console.log('Task nodes', tasks_nodes.filter(task => task.nodeName === 'LI'))   
        
        
    let tasks = Array.from(task_list.children).map(task => {
            return {
...
function addToLocalStorage() {
    var fromstorage = document.getElementById('from-storage')

    localStorage.setItem('taskslist', 'hello world')

    fromstorage.textContent = localStorage.getItem('task')
}